...
NOT ported. The study repo has a `rtlgen/config.py` of 254 lines and none of it belongs here --
...
clock. A generator has no sweep, no part and no clock. Copying it would import the study's
...
class Pipeline:
    ...
    Defaults match the study repo's shipped depth, which was chosen by measurement rather than
...
class BuildConfig:
    ...
    that cost the study repo six separate crashes.
    """
