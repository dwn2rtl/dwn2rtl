...
REWRITTEN, not ported. The earlier implementation's `tb/gen_vectors.py` mixed 500 random vectors with real
...
The earlier implementation's real-data half caught nothing its random half did not.
...
def _top_vectors(thr_q, used, z, n_features, precision, rng, n_random):
    ...
    WHERE THE RANGE COMES FROM. The earlier implementation drew random features between the min and max of
...
def generate(ck, layers, outdir, precision, n_random=N_RANDOM, seed=SEED):
    ...
    write_lines(os.path.join(outdir, 'vec_params.vh'), [
        ...
        # Width of class_idx. The earlier implementation's testbenches hardcoded 3 -- one dataset's class
